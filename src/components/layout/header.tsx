...
import { Sparkles, LayoutGrid, LogOut, Plus, Globe, Settings } from 'lucide-react';
...
export function Header({ onNewBoard, totalBoards = 0, totalWorkspaces = 0 }: HeaderProps) {
  const { user, isAuthenticated, logout, loginWithGoogle, updateProfile } = useAuth();
        ...
        <div className="flex items-center gap-6">
          ...
          {isAuthenticated && (
            <nav className="hidden md:flex items-center gap-1 text-sm">
              <Link
                href="/dashboard"
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-white/5 transition-colors font-medium"
              >
          ...
          {isAuthenticated ? (
            <>
              ...
              <div className="flex items-center gap-3 pl-3 border-l border-white/10">
                {/* User Avatar & Name button (opens Account Settings) */}
                <button
                ...
                >
                  {user?.avatarUrl ? (
                    ...
                    <img
                      ...
                      className="w-8 h-8 rounded-full border border-indigo-500/50 bg-slate-800 object-cover"
                    />
                  ) : (
                  ...
                  <span className="hidden sm:inline-block text-xs font-semibold text-slate-200 group-hover:text-indigo-300 transition-colors">
                    {user?.displayName}
                  </span>
                </button>

                {/* Account Settings Gear Icon Button */}
                <button
                  onClick={() => setIsSettingsOpen(true)}
                  title="Ajustes de Cuenta"
                  className="p-2 text-slate-400 hover:text-indigo-400 hover:bg-white/5 rounded-xl transition-colors"
                >
