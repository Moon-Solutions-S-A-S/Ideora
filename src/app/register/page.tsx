...
import { Sparkles, ArrowRight, ArrowLeft, Lock, Mail, User, Globe } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-slate-950 px-4 py-12 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-emerald-600/15 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md glass-panel rounded-3xl border border-white/10 p-8 shadow-2xl relative z-10 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">{t('reg_subtitle')}</p>
        </div>
        ...
        <div className="relative my-4">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-slate-900 px-3 text-slate-400 font-medium">{t('reg_or_form')}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          >
            {loading ? 'Creating account...' : t('reg_btn_submit')}
